...
const createClientSchema = z.object({
  ...
  age: z.number().int().min(10, 'La edad mínima es 10 años').max(100, 'La edad máxima es 100 años'),
  phone: z.string().regex(/^\d{10}$/, 'El teléfono debe tener 10 dígitos (formato mexicano)'),
  email: z.string().email('El correo no es válido').or(z.literal('')).optional(),
  rfc: z.string().regex(/^[A-ZÑ&]{3,4}\d{6}[A-Z0-9]{3}$/i, 'El RFC no tiene un formato válido'),
  ...
  payment_amount: z.number().positive('El monto de pago debe ser positivo'),
