...
const renderEventCard = (ev, onEventClick) => {
  ...
  return (
    ...
    >
      ...
      <span className="font-normal text-[9.5px] shrink-0 text-[var(--color-text-muted)] leading-none">{timeStr}</span>
...
export const AgendaCalendar = ({
  ...
  return (
    <div className="w-full overflow-x-auto custom-scrollbar">
      <div className="grid grid-cols-[repeat(6,minmax(0,1fr))_minmax(0,0.65fr)] gap-1 min-w-[900px]">
        ...
        {monthDays.map((d) => {
          ...
          return (
            <div
              ...
              className={`rounded-sm p-1 border transition-all flex flex-col min-h-[230px] h-64 cursor-pointer ${isSelected
            ...
            >
              <div className="flex justify-between items-center mb-1">
                <div className={`text-xs font-bold flex items-center justify-center w-6 h-6 rounded-full ${isToday ? 'bg-[var(--color-gold)] text-white shadow' : ''} ${!isCurrentMonth ? 'text-[var(--color-text-muted)]' : 'text-[var(--color-text)]'}`}>
                  {d.getDate()}
                </div>
                <button
                  type="button"
